...
class DatasetVisualizer:
    ...
    def plot_column_frequency(self, col_name: str, plot_title: str, top_n: int = None, df: pd.DataFrame = None) -> None:
        ...
        value_counts = df[col_name].value_counts(dropna=False).head(top_n)
    
        # Save the value counts to a CSV file
        value_counts.to_csv(f'{col_name}_frequency_counts.csv')

        # Plot the data
        value_counts.plot(kind='bar', title=plot_title, xlabel=col_name, ylabel='Count')
        plt.show()
    ...
    def plot_column_frequency_stacked_proportion(self, col_name: str, xlabel: str, plot_title: str, df: pd.DataFrame = None, top_n: int = None) -> None:
        ...
        # data with top_n filtering
        col_data = df[col_name].value_counts(normalize=True, dropna=False).head(top_n)

        # Save the proportional value counts to a CSV file
        col_data.to_csv(f'proportional_value_counts_{col_name}.csv', header=True)
    ...
    def plot_grouped_columns_frequency(self, pairs: list, question_category: str) -> None:
        ...
        # Create a DataFrame with the sorted values and labels
        sorted_data = pd.DataFrame({question_category: sorted_labels, 'Count': sorted_values})

        # Save the sorted data to a CSV file
        sorted_data.to_csv(f'{question_category}_grouped_frequencies.csv', index=False)
    ...
    def plot_proportional_stacked_bar(self, col1: str, col2: str, title: str, df: pd.DataFrame = None) -> None:
        ...
        # Save the proportional contingency table to a CSV file
        contingency_table.to_csv(f'proportional_table_{col1}_vs_{col2}.csv')
    ...
    def plot_frequency_distribution_stacked_unstacked_subplot(
        ...
        contingency_table.to_csv(f'contingency_table_{col1}_vs_{col2}__Counts.csv')
